...
def LichenCII_v3_bursty(Mvec, MLpar, z):
    ...
    logMb = np.unique(x[:,1])
    logSFRb = x[:,2].reshape(137,122,order='F')
    logSFR_interp = interp2d(logMb, zb, logSFRb, 
    ...
    delta = lambda z: (3.508+ (2.608*(a(z)-1)-0.043*z)*nu(z))
    gamma = lambda z: (0.316+ (1.319*(a(z)-1)+0.279*z) * nu(z))
    f = lambda x,z: (-np.log10(10**(alpha(z)*x)+1)+ delta(z)* 
        (np.log10(1+np.exp(x)))**gamma(z)/ (1+np.exp(10**(-x))))
    xi = lambda z: 0.218 - 0.023*(a(z)-1)

    def stellar_m(halo_m, z, scatter=False):
        sm = 10**(log10_eps(z)+ log10_M1(z)+ 
            f(np.log10((halo_m/(10**log10_M1(z))).value),z)- f(0,z))
        if scatter:
            rand = np.random.lognormal(-0.5*(xi(z)*np.log(10))**2,
                                    xi(z)*np.log(10))
            return sm * rand
        else:
            return sm
    ...
    sigma_dex = sigma_logL_with_epsilon(Mvec.to(u.Msun).value,
                                        epsilon=epsilon)
    ...
    scatter = np.exp(np.random.normal(loc=-0.5 * sigma_ln**2,
                    scale=sigma_ln,size=Mvec.shape))
